...
const SEEDS_A = [
  "pico e vale", "padrão invisível", "energia", "coragem", "prioridade real", "microvitórias",
  "gentileza", "limite", "foco", "descanso", "ambiente", "autocrítica", "curiosidade",
  "rotas de fuga", "atrito", "alívio", "procrastinação útil", "1% melhor", "fricção", "expectativa",
  "frustração", "orgulho", "aprendizado", "pedido de ajuda", "não feito", "feito", "leveza",
  "intenção", "autonomia", "cuidado"
];
const SEEDS_B = [
  "manhã", "tarde", "noite", "rotina", "agenda", "corpo", "casa", "trabalho", "relacionamentos",
  "sozinho", "ruído", "silêncio", "cansaço", "âncora", "ritual", "movimento", "trava", "desbloqueio",
  "prazo", "medo", "alegria", "raiva", "tristeza", "ansiedade", "satisfação", "tédio", "impulso",
  "hábito", "planejamento", "execução"
];
const TEMPLATES = [
    ...
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((n) => (
        <button
          key={n}
          onClick={() => onChange?.(n)}
          className={`p-0.5 rounded ${value >= n ? "text-yellow-400" : "text-zinc-300"}`}
        >
...
export default function EmotionalPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      mList.forEach((r) => {
        ...
        if (arr.length) map[r.dateKey] = arr.reduce((a, b) => a + b, 0) / (arr.length * 4);
      });
      jList.forEach((r) => {
        if (!map[r.dateKey]) {
  ...
  const habitIcons = {
    Água: <Droplet className="w-4 h-4" />,
    Exercício: <Bike className="w-4 h-4" />,
    Leitura: <BookOpen className="w-4 h-4" />,
    Meditação: <BrainCircuit className="w-4 h-4" />,
  };
        ...
        {moodEmojis.map((emoji, i) => {
          ...
          return (
            <button
              key={i}
              onClick={() => onChange(i)}
              className={`h-11 w-11 grid place-items-center rounded-full border ${
                active ? "border-yellow-400 bg-yellow-50" : "border-[var(--pc-border)]"
              }`}
            >
  ...
  return (
    <motion.div key={dateISO} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="grid gap-6">
      ...
      <SectionCard title="Sono" icon={<span>💤</span>}>
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="flex flex-col">
            ...
            <StarRating value={sleep?.quality || 0} onChange={(v) => setSleep({ ...sleep, quality: v })} size={20} />
          </div>
        </div>
      </SectionCard>
      ...
      <SectionCard
        ...
        icon={<span>✓</span>}
        right={
          <button onClick={() => setOpenHabits(true)} className="p-2 rounded-full">
      ...
      >
        <div className="flex flex-wrap gap-2">
          {habits.map((h) => {
            ...
            return (
              <button key={h} onClick={() => setCheck({ ...(check || {}), [h]: !active })} className={`rounded-full px-4 py-2 border ${active ? "bg-emerald-50" : ""}`}>
                {habitIcons[h] || <Star className="w-4 h-4" />}
                {h}
              </button>
            );
          })}
